...
import 'package:enationn/src/data/repos/remote/jobRepository/job_account_repository.dart';
...
class _CandidateSectionState extends State<CandidateSection> {
  ...
  void checkStatus() async {
    jobAccount = await JobAccountRepository().getJobsData();
    if (!mounted) return;
...
CRTD Technologies""";
  ...
  Future<bool> _applyForJob(UserProvider uProvider) async {
    return await JobAccountRepository().postJobsData(
      uProvider.uId,
      uProvider.fullName,
      uProvider.email,
      uProvider.college,
