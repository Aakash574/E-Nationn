...
import 'package:enationn/src/data/repos/remote/jobRepository/job_repository.dart';
...
import '../ExtraScreens/plan_Screen.dart';
import '../PopScreens/subscribe_Pop_Up.dart';
...
class _DashboardState extends State<Dashboard> {
  ...
  Future<void> details() async {
    event = await EventRepository().getEventDetails();
    hackathon = await HackathonRepository().getHackathonDetails();
  ...
  void setUserCredentials(BuildContext context) async {
    final userProvider = Provider.of<UserProvider>(context, listen: false);
    final loginCredentials = await SharedPref().getUserCredentials();
    final getUser = await UserRepository().getUserDataById(
      int.parse(loginCredentials['id']),
    );
    ...
    userProvider.setState(getUser['state']);
    ...
    userProvider.setInternshipStatus(getUser['internship_status']);
  ...
  sliderEachSection(
    ...
    return !toShow
        ...
        : Image.asset(
            imagePath.isEmpty
                ? 'https://wallpaperaccess.com/full/2903164.jpg'
                : imagePath,
          );
  }
}
