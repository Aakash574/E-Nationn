...
import 'package:enationn/src/data/repos/remote/userEventRepository/user_event_repository.dart';
import 'package:enationn/src/data/repos/remote/userEventRepository/user_hackathon_repository.dart';
import 'package:enationn/src/data/repos/remote/userEventRepository/user_internship_repository.dart';
import 'package:enationn/src/data/repos/remote/userRepository/basic_key_repository.dart';
import 'package:enationn/src/data/repos/remote/userRepository/user_repository.dart';
import 'package:enationn/src/data/repos/remote/voucherCodeRepository/voucher_code_repository.dart';
import 'package:flutter/material.dart';
import 'package:intl/intl.dart';
import 'package:provider/provider.dart';

import '../../../../const.dart';
import '../../../data/repos/remote/userRepository/premium_key_repository.dart';
...
""";
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: TextButton(
                          onPressed: () async {
                            // ------Validating Voucher ------->
                            log("Current Plan Status :: ${userDataProvider.planStatus.toString()}");
                            if (userDataProvider.planStatus != 'premium' &&
                                userDataProvider.planStatus != 'basic') {
                              log("Voucher Code Repository -------->");
                              if (await VoucherCodeRepository()
                              ...
                              switch (userDataProvider.planStatus) {
                                case 'basic':
                                  ...
                                  if (await PremiumPasskeyRepository()
                                ...
                                case 'premium':
                                  ...
                                  if (await BasicPasskeyRepository()
                                      .passkey(voucherController.text)) {
                                    planStatus(userDataProvider, basicVariable);
                                  }
                                  break;
  ...
  void eventStatus(UserProvider userDataProvider) async {
    ...
    final isRegister = await UserEventRepository().eventRegistration(
  ...
  void hackathonStatus(HackathonProvider hackathonDataProvider,
    ...
    sendEmail(userDataProvider.fullName);
    final isRegister = await UserHackathonRepository().hackathonRegistration(
      widget.events[widget.index]['name'],
      userDataProvider.fullName,
      userDataProvider.email,
  ...
  void internshipStatus(UserProvider userDataProvider) async {
    sendEmail(userDataProvider.fullName);
    List internshipData = await InternshipRepository().getInternshipDetails();

    log(internshipData.toString());

    log("\n \n Internship Section \n \n");
    final isRegister = await UserInternshipRepository().internshipRegistration(
      userDataProvider.fullName,
