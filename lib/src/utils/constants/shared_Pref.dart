...
class SharedPref {
  ...
  Future<Map<dynamic, dynamic>> getUserData(int id) async {
    final userData = await UserRepository().getUserDataById(id);
    return userData;
  }
  ...
  Future<bool> authenticateUser(
    ...
    final id = await UserRepository().getUserDataByEmail(email, 'id');
    final userData = await UserRepository().getUserDataById(int.parse(id));
    log("ID :: $id");
